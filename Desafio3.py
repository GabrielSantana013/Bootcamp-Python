...
def cadastrar_conta(numero_conta, clientes, contas):
    ...
    limite_saque = int(input("Qual o máximo de saques que podem ser feitos em um dia? "))
...
def sacar(clientes):
    ...
    valor = float(input(f"Digite o valor que deseja sacar (máximo = {conta.get_limite}): "))
    transacao = Saque(valor)
    cliente.realizar_transacao(conta, transacao)
    conta.registrar_saque()
    print(f"Você ainda tem {limite_conta - saques_realizados - 1} saques disponíveis")
...
# conta = Conta.nova_conta("Cliente Exemplo", 123)
# cliente = Cliente("Endereço Exemplo")
